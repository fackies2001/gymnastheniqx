...
class DatatableServices
{
     ...
     */
    /*
    public function get_supplier_product_show_table($request)
    {
        ...
        $query = \App\Models\SupplierProduct::with(['category', 'supplier'])->where('supplier_id', $supplierId);
    ...
    /**
     * ✅ UPGRADED: Beautiful UI with color coding and tooltips
     */
    /*
    public function get_serialized_products_summary_table($query)
    {
        return DataTables::eloquent($query)
            ...
            ->addColumn('quantity', function ($row) {
                ...
                $damaged = $row->damaged_count ?? 0;
                $lost = $row->lost_count ?? 0;
                $total = $available + $reserved + $sold + $damaged + $lost;

                // ✅ Determine badge style based on available stock
                if ($available > 20) {
                ...
                if ($reserved > 0) {
                    $tooltipLines[] = "🔒 Reserved: {$reserved}";
                }
                if ($sold > 0) {
                    $tooltipLines[] = "💰 Sold: {$sold}";
                }
                if ($damaged > 0) {
                    $tooltipLines[] = "⚠️ Damaged: {$damaged}";
                }
                if ($lost > 0) {
                    $tooltipLines[] = "❌ Lost: {$lost}";
                }

                $tooltipLines[] = "━━━━━━━━━━━━━━━━";
                $tooltipLines[] = "📊 Total Items: {$total}";

                $tooltip = implode("\n", $tooltipLines);

                // ✅ Build badge HTML
                return '<div class="text-center" data-toggle="tooltip" data-html="true" 
                         data-placement="left" 
                         title="' . htmlspecialchars($tooltip) . '" 
                         style="cursor: help;">
                        <span class="badge ' . $badgeClass . ' ' . $textClass . ' px-3 py-2" 
                              style="font-size: 1.1rem; font-weight: 600; min-width: 100px; display: inline-flex; align-items: center; justify-content: center; gap: 8px;">
